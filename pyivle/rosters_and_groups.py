import api

# Rosters_and_Groups.Class_Roster
def class_roster(courseId, auth=True):
    params = {'CourseID': courseId}
    return api.call('Class_Roster', params, auth)

# Rosters_and_Groups.Guest_Roster
def guest_roster(courseId, auth=True):
    params = {'CourseID': courseId}
    return api.call('Guest_Roster', params, auth)

# Rosters_and_Groups.GroupsByUserAndModule
# TODO: test this
def groups_by_user_and_module(courseId=None, acadYear=None, semester=None, auth=True):
    if courseId is None and (None in [acadYear, semester]):
        raise api.InvalidParametersException('If courseId is empty, then acadYear and semester cannot be empty.')
    else:
        params = {'CourseID': courseId, 'AcadYear': acadYear, 'Semester': semester}
    return api.call('GroupsByUserAndModule', params, auth)

# Rosters_and_Groups.GroupsByUser
def groups_by_user(auth=True):
    return api.call('GroupsByUser', {}, auth)

# Rosters_and_Groups.Module_ClassGroups
def module_class_groups(courseId, auth=True):
    params = {'CourseID': courseId}
    return api.call('Module_ClassGroups', params, auth)

# Rosters_and_Groups.Module_ClassGroupUsers
def module_class_group_users(classGroupId, auth=True):
    params = {'ClassGroupID': classGroupId}
    return api.call('Module_ClassGroupUsers', params, auth)

# Rosters_and_Groups.Module_OfficialGroupUsers
# TODO: test this
def module_official_group_users(courseId=None, moduleCode=None, groupName=None, acadYear=None, semester=None, groupType=None, auth=True):
    if courseId and (not groupName or not groupType):
        raise api.InvalidParametersException('If courseId is given, then groupName and groupType cannot be empty.')
    elif courseId is None and (None in [moduleCode, groupName, acadYear, semester, groupType]):
        raise api.InvalidParametersException('If courseId is empty, then moduleCode, groupName, acadYear, semester, and groupType cannot be empty.')
    else:
        params = {'CourseID': courseId, 'ModuleCode': moduleCode, 'GroupName': groupName, 'AcadYear': acadYear, 'Semester': semester, 'GroupType': groupType}
    return api.call('Module_OfficialGroupUsers', params, auth)

# Rosters_and_Groups.ClassGroupSignUp_JSON
def class_group_sign_up(courseId, groupId, auth=True):
    params = {'CourseID': courseId, 'GroupID': groupId}
    return api.call('ClassGroupSignUp_JSON', params, auth, 'post')

# Rosters_and_Groups.ClassGroupSignUpRemove_JSON
def class_group_sign_up_remove(courseId, groupId, auth=True):
    params = {'CourseID': courseId, 'GroupID': groupId}
    return api.call('ClassGroupSignUpRemove_JSON', params, auth, 'post')

# Rosters_and_Groups.GroupProjectsByUser
def group_projects_by_user(courseId, includeGroups, groupType, auth=True):
    params = {'CourseID': courseId, 'IncludeGroups': includeGroups, 'GroupType': groupType}
    return api.call('GroupProjectsByUser', params, auth)

# Rosters_and_Groups.Project_SelfEnrolGroups
def project_self_enrol_groups(projectId, auth=True):
    params = {'ProjectID': projectId}
    return api.call('Project_SelfEnrolGroups', params, auth)

# Rosters_and_Groups.ProjectGroupUsers
def project_group_users(projectGroupId, auth=True):
    params = {'ProjectGroupID': projectGroupId}
    return api.call('ProjectGroupUsers', params, auth)

# Rosters_and_Groups.Project_EnrolledGroups
def project_enrolled_groups(auth=True):
    return api.call('Project_EnrolledGroups', {}, auth)

# Rosters_and_Groups.ProjectGroup_SignUp_JSON
def project_group_sign_up(projectGroupId, auth=True):
    params = {'ProjectGroupID': projectGroupId}
    return api.call('ProjectGroup_SignUp_JSON', params, auth, 'post')

# Rosters_and_Groups.ProjectGroup_RemoveSignUp_JSON
def project_group_remove_sign_up(projectGroupId, auth=True):
    params = {'ProjectGroupID': projectGroupId}
    return api.call('ProjectGroup_RemoveSignUp_JSON', params, auth, 'post')
